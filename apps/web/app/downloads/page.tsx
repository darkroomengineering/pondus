	...
	const platformDownloads = [
		{
			...
			variants: [
				{ name: 'Apple Silicon', asset: release?.assets.macos.arm64 ?? null },
				{ name: 'Intel', asset: release?.assets.macos.x64 ?? null },
			],
		},
		{
			platform: 'Windows',
			...
			variants: [
				{ name: 'Installer (exe)', asset: release?.assets.windows.exe ?? null },
				{ name: 'MSI', asset: release?.assets.windows.msi ?? null },
			],
		},
	]
							...
							<Card.Content className="py-8">
								...
								<div className="space-y-3">
									{platform.variants.map((variant, idx) => {
											...
											<>
												<div>
													<p className="font-medium flex items-center gap-2">
														{variant.name}
														{!isAvailable && (
															<span className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--card)] text-[var(--muted)]">
																Coming soon
															</span>
														)}
													</p>
												...
												{isAvailable ? (
													<svg
														className="w-5 h-5 text-[var(--muted)] group-hover:text-[var(--accent)] transition-colors"
														fill="none"
														stroke="currentColor"
														viewBox="0 0 24 24"
													>
										...
										return asset ? (
											<a key={asset.name} href={asset.url} className={className}>
												{content}
											</a>
										) : (
						...
						<Card.Content className="py-8">
							<div className="flex items-start justify-between mb-6">
								<div>
									...
									<p className="text-[var(--muted)]">Unlock unlimited history, exports, and more</p>
								</div>
